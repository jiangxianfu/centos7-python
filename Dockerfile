...
RUN cd /tmp && curl https://www.python.org/ftp/python/2.7.18/Python-2.7.18.tgz \
    && tar zxf Python-2.7.18.tgz && cd Python-2.7.18 && ./configure && make && make install
RUN mv /usr/bin/python2.7 /usr/bin/python2.7.5 && ln -s /usr/local/bin/python2.7 /usr/bin/python2.7
RUN cd /tmp && curl https://bootstrap.pypa.io/get-pip.py && python get-pip.py
RUN set -ex \
